"""
Cement custom redis extension module.
"""
...
class CustomRedisCacheHandler(cache.CacheHandler):
    ...
    **Note** This extension has an external dependency on ``redis``.  You
    ...
    def __init__(self, *args, **kw):
        super(CustomRedisCacheHandler, self).__init__(*args, **kw)
        self.mc = None

    def _setup(self, *args, **kw):
        super(CustomRedisCacheHandler, self)._setup(*args, **kw)
    ...
    def delete_by_pattern(self, pattern):
        """
        Delete all keys from the cache for the given pattern

        Args:
            pattern (str): The pattern to delete keys from the cache.
